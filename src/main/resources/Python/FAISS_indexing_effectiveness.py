...
def indexing(nchange, nskip):
    ...
    changes_feature_vectors = pd.read_csv('./src/main/resources/Features_Vectors/changes_feature_vectors_java.csv',
                                          header=None, skiprows=int(nskip), nrows=int(nchange)).iloc[:, :].values[0:,
                              :-1].astype('float32')
