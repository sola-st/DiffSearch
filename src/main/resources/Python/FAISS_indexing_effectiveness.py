...
def indexing(nchange, nskip):
    #Reading csv feature vectors files
    changes_feature_vectors = pd.read_csv('./src/main/resources/Features_Vectors/changes_feature_vectors_java.csv', header=None,skiprows=int(nskip), nrows=int(nchange)).iloc[:, :].values[0:, :-1].astype('float32')
...
indexing(sys.argv[-2],sys.argv[-1])