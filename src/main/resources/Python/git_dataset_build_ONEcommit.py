...
def perform_commit(r, files, commit, commit_message):
    ...
    isExist = os.path.exists('./java_top100_dataset/dataset/'+repo)

    if not isExist:
        # Create a new directory because it does not exist 
        os.makedirs('./java_top100_dataset/dataset/'+repo)
    ...
    if FIRST_COMMIT:
        ...
        for file in files:
            ...
            temp = file.split("/")
            with open('./java_top100_dataset/dataset/'+repo + '/' + temp[-1], "w") as myfile:
                myfile.write(lines)
